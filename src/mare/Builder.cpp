...
bool Builder::build(const Map<String, String>& userArgs)
{
  ...
  engine.enterDefaultKey("cppSource");
    ...
    engine.addResolvableKey("command", "$(CXX) -MMD $(__soFlags) -o $(ofile) -c $(file) $(cppFlags) $(CXXFLAGS) $(patsubst %,-D%,$(defines)) $(patsubst %,-I%,$(includePaths))");
  ...
  engine.enterDefaultKey("cSource");
    ...
    engine.addResolvableKey("input", "$(file) $(filter-out %.o: \\,$(readfile $(dfile)))");
    engine.addResolvableKey("output", "$(ofile) $(dfile)");
    engine.addResolvableKey("command", "$(CC) -MMD $(__soFlags) -o $(ofile) -c $(file) $(cFlags) $(CFLAGS) $(patsubst %,-D%,$(defines)) $(patsubst %,-I%,$(includePaths))");
    engine.addResolvableKey("message", "$(file)");
  engine.leaveKey();
#if defined(_WIN32)
